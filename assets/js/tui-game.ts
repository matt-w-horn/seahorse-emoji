...
interface Bullet { x: number; y: number; z: number; vz: number; }
interface Star { x: number; y: number; z: number; }
interface Spark { x: number; y: number; z: number; t: number; }
interface Cam { x: number; y: number; vx: number; vy: number; }
interface Palette { bg: string; fg: string; accent: string; dim: string; font: string; }
...
// hitBullet/hitShip read only a few fields, so they accept the minimal shape
// (the game passes full Rock/Bullet; tests pass bare {x,y,z,size} literals).
...
export function start(canvas: HTMLCanvasElement, opts: GameOpts): GameHandle {
  ...
  /* rendering */
  function drawPoly(cx: number, cy: number, rock: Rock) {
    var S = SIZES[rock.size];
    ctx.beginPath();
  ...
  function drawIcoCentered(cx: number, cy: number, scale: number, t: number) {
    ctx.beginPath();
    for (var i = 0; i < IEDGES.length; i++) {
      var e = IEDGES[i];
  ...
  function render() {
    ...
    if (fireCool > 0.14) {
      ctx.strokeStyle = pal.fg;
      ctx.beginPath();
      ([[-1, -1], [1, -1], [-1, 1], [1, 1]] as Edge[]).forEach(function (d) {
    ...
    if (invuln <= 0 || Math.floor(invuln * 8) % 2 === 0) {
      ctx.strokeStyle = pal.accent;
      ctx.beginPath();
      ([[-1, 0], [1, 0], [0, -1], [0, 1]] as Edge[]).forEach(function (d) {
